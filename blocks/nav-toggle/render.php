...
$overlay_id = $block->context['snap-megamenu/overlayId'] ?? '';
...
$aria_label    = '' !== $label ? $label : __( 'Open menu', 'snap-megamenu-builder' );
$aria_controls = '' !== $overlay_id ? ' aria-controls="' . esc_attr( $overlay_id ) . '"' : '';

$icon_markup = sprintf(
	'<span class="snap-nav-toggle__icon snap-nav-toggle__icon--%s" aria-hidden="true">
		<span></span>
		<span></span>
		<span></span>
	</span>',
...
if ( $label_visible && '' !== $label ) {
	$label_markup = sprintf(
		'<span class="snap-nav-toggle__label">%s</span>',
		esc_html( $label )
	);
